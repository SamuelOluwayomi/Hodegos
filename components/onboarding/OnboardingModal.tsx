...
export default function OnboardingModal({ open, onClose, walletAddress }: OnboardingModalProps) {
  ...
  const handleLevelSelect = (level: 'beginner' | 'intermediate' | 'master') => {
    ...
    if (level === "beginner") {
      sendMessage(
        `[SYSTEM] User selected BEGINNER level. Start teaching them trading fundamentals step by step. Begin with: "What is trading?" Use simple language, real-world analogies. Wait for the user to answer or confirm they understand before moving to the next topic. Topics to cover in order: 1) What is trading 2) What are crypto exchanges 3) Trading pairs 4) Market vs Limit orders 5) Reading charts basics 6) Risk management 7) Spot vs Perpetual. Cover ONE topic at a time. Prefix each new topic with "Topic X of 7: [Name]" so they know their progress. Keep it digestible. Provide a visual for each topic using standard markdown image syntax: ![description](/filename). Use EXACTLY these image paths: Topic 1: /trading_intro.png, Topic 2: /crypto_exchanges.png, Topic 3: /trading_pairs.png, Topic 4: /market_limit.png, Topic 6: /risk_management.jpg, Topic 7: /spot_perpetual.png. NEVER use the [IMAGE_PLACEHOLDER] tag.`
      );
      goToStep("learn");
    } else {
      sendMessage(
