...
export default function OnboardingModal({ open, onClose, walletAddress }: OnboardingModalProps) {
  ...
  const handleLevelSelect = (level: 'beginner' | 'intermediate' | 'master') => {
    ...
    if (level === "beginner") {
      sendMessage(
        `[SYSTEM] User selected BEGINNER level. Start teaching them trading fundamentals step by step. Begin with: "What is trading?" Use simple language, real-world analogies. Wait for the user to answer or confirm they understand before moving to the next topic. Topics to cover in order: 1) What is trading 2) What are crypto exchanges 3) Trading pairs 4) Market vs Limit orders 5) Reading charts basics 6) Risk management 7) Spot vs Perpetual. Cover ONE topic at a time. Prefix each new topic with "Topic X of 7: [Name]" so they know their progress. Keep it digestible. You MUST provide a visual for EVERY topic using standard markdown image syntax: ![description](/filename). Use EXACTLY these image paths for each topic: Topic 1: /trading_intro.png, Topic 2: /crypto_exchanges.png, Topic 3: /trading_pairs.png, Topic 4: /market_limit.png, Topic 5: /reading_charts.png, Topic 6: /risk_management.jpg, Topic 7: /spot_perpetual.png. ALWAYS include the image for the current topic. NEVER use the [IMAGE_PLACEHOLDER] tag.`
      );
      goToStep("learn");
    } else {
      sendMessage(
        `[SYSTEM] User selected ${level.toUpperCase()} level. Ask them ${level === "intermediate" ? "3 intermediate" : "3 advanced"} trading questions to verify their knowledge. Ask ONE question at a time. Prefix each question with "Question X of 3". Mix multiple-choice and explain questions — use [MCQ] tags for multiple choice (with exactly 4 options A/B/C/D) and [EXPLAIN] tags for open-ended questions. DO NOT output or reveal the correct answer when asking a question; wait for the user to answer first. If they answer correctly, award XP. After all 3 questions, summarize their score. If they got the majority right, explicitly say "You passed!" so they can proceed. If they failed, explicitly say "You didn't pass this time, let's review" and offer to try again.`
      );
      goToStep("quiz");
    }
  };
